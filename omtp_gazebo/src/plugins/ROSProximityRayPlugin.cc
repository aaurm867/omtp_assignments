...
#include "omtp_gazebo/plugins/ROSProximityRayPlugin.hh"
...
void ROSProximityRayPlugin::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  this->statePub = this->rosnode->advertise<omtp_gazebo::Proximity>(
    this->stateTopic, 1, true);
  this->stateChangePub = this->rosnode->advertise<omtp_gazebo::Proximity>(
